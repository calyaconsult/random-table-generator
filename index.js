const fs = require('fs');
const monthnames = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"].map((x) => { return x.substr(0, 3); });
const d = new Date();
...
var columns = ["Monat", "Monat Text", "Unique Users", "Visits", "Pageviews", "Pageviews/Visit", "Avg. Time on Page", "Avg. Session Duration", "Avg. Read Time", "Use Time", "Field Type"];
var types = ["string", "string", "int", "int", "int", "float(2)", "float(2)", "float(2)", "hms", "hms", "string"];
...
const fillWithRandVals = (a) => {
  ...
  return () => {
    ...
    return arr.map((x) => { return Math.floor(Math.random() * 1000); });
  };
};
...
var rowsInner;
var table = [[columns[0], currentYear, previousYear, "Veränderung", "{BLANK}", ...Array(3).fill(currentYear), "{BLANK}", ...Array(3).fill(previousYear), "{BLANK}", ...Array(3).fill("Veränderung")]];

for (let c = 1; c < columns.length - 1; c++) {
  for (let g = 0; g < groups.length; g++) {
    switch (groups[g]) {
      case "total":
        if (c >= 2) { // Die Bedingung c >= 2 scheidet die ersten beiden von den übrigen Spalten; in unserer Tabelle befinden sich in Spalte 1 und 2 Beschriftungen, in den übrigen Spalten die Daten
          rowsInner = [columns[c], Array(3).fill("calc"), "{BLANK}"]; break;
        } else {
          rowsInner = [columns[c], `${month} ${currentYear}`, `${month} ${previousYear}`, "Total", "{BLANK}"];
          break;
        }
      case "devices":
        if (c >= 2) {
          rowsInner = [...rowsInner, Array(3).fill("calc"), "{BLANK}", Array(3).fill("calc"), "{BLANK}", Array(3).fill("calc")]; break;
        } else {
          rowsInner = [...rowsInner, ...devices, "{BLANK}", ...devices, "{BLANK}", ...devices]; break;
        }
      default:
    }
  }
  table.push(rowsInner.map((x) => { return fillWithRandVals(x)(); }).flat());
}
table.push([table[0].map((x) => { // Als letztes wird eine Spalte angehängt, die Metainformationen zu den einzelnen Zeilen enthält
  switch (x) {
    case table[0][0]: return columns[columns.length - 1];
    case "{BLANK}": return "{BLANK}";
    case "Veränderung": return "calculated";
...
var calcRows = table[0].map((x, i) => { if (x == "Veränderung") return i; }).filter((x) => { return x }); // Welche Zeilen nehmen berechnete Ergebnisse auf?
